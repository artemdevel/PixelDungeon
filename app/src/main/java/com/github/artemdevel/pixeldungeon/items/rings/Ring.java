 ...
 */
package com.github.artemdevel.pixeldungeon.items.rings;
...
import com.github.artemdevel.pixeldungeon.items.ItemStatusHandler;
...
public class Ring extends EquipableItem {
    ...
    public boolean doEquip(final Hero hero) {
        if (hero.belongings.ring1 != null && hero.belongings.ring2 != null) {
            ...
            if (cursed) {
                equipCursed(hero);
                GLog.logNegative("your " + this + " tightens around your finger painfully");
            }
    ...
    public class RingBuff extends Buff {
        ...
        public boolean act() {
            if (!isIdentified() && --ticksToKnow <= 0) {
                String gemName = name();
                identify();
                GLog.logWarning(TXT_IDENTIFY, gemName, Ring.this.toString());
                Badges.validateItemLevelAcquired(Ring.this);
            }
            use();
            spend(TICK);
